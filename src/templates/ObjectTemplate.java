// This template implements a class for working with a Rust struct via a long/Arc<T>
// to the live Rust struct on the other side of the FFI.
//
// Each instance implements core operations for working with the Rust `Arc<T>` and the
// Java long pointer to work with the live Rust struct on the other side of the FFI.
//
// There's some subtlety here, because we have to be careful not to operate on a Rust
// struct after it has been dropped, and because we must expose a public API for freeing
// the Java wrapper object in lieu of reliable finalizers. The core requirements are:
//
//   * Each instance holds an opaque pointer (as a long) to the underlying Rust struct.
...
{%- endif %}
  ...
  public {{ impl_class_name }}(long pointer) {
  ...
  public {{ impl_class_name }}({% call java::arg_list(cons, true) -%}) {% match cons.throws_type() %}{% when Some(throwable) %}throws {{ throwable|type_name(ci, config) }}{% else %}{% endmatch %}{
    this((long){%- call java::to_ffi_call(cons) -%});
  }
  {%-     endif %}
  {%- when None %}
  {%- endmatch %}
  ...
  private class UniffiCleanAction implements Runnable {
    ...
    public UniffiCleanAction(long pointer) {
      this.pointer = pointer;
    }
    ...
    public void run() {
      if (pointer != 0L) {
        UniffiHelpers.uniffiRustCall((_arena, status) -> {
          UniffiLib.{{ obj.ffi_object_free().name() }}(pointer, status);
          return null;
        });
      }
    }
  ...
  long uniffiClonePointer() {
    return UniffiHelpers.uniffiRustCall((_arena, status) -> {
      if (pointer == 0L) {
        throw new NullPointerException();
      }
      return UniffiLib.{{ obj.ffi_object_clone().name() }}(pointer, status);
    });
  }
...
public class {{ impl_class_name }}ErrorHandler implements UniffiRustCallStatusErrorHandler<{{ impl_class_name }}> {
    @Override
    public {{ impl_class_name }} lift(MemorySegment error_buf) {
