...
impl JavaCodeOracle {
    ...
    /// FFI type name to use inside FFI structs.
    /// In FFM mode, most compound types are MemorySegment.
    fn ffi_type_label_for_ffi_struct(
        ...
        self.ffi_type_label_by_value(ffi_type, true, config, ci)
    }

    /// Default values for FFI
    ///
    /// This is used to set a default return value for error results.
    fn ffi_default_value(&self, ffi_type: &FfiType) -> String {
        match ffi_type {
            ...
            FfiType::RustArcPtr(_) => "0L".to_owned(),
            FfiType::RustBuffer(_) => "MemorySegment.NULL".to_owned(),
            FfiType::Callback(_) => "MemorySegment.NULL".to_owned(),
            FfiType::RustCallStatus => "MemorySegment.NULL".to_owned(),
            FfiType::VoidPointer => "MemorySegment.NULL".to_owned(),
            FfiType::Struct(_) => "MemorySegment.NULL".to_owned(),
            FfiType::ForeignBytes => "MemorySegment.NULL".to_owned(),
    ...
    fn ffi_type_label_by_reference(
        ...
        // In FFM, all by-reference types are passed as MemorySegment
    ...
    fn ffi_type_label(
        ...
        match ffi_type {
            ...
            FfiType::RustArcPtr(_) => "Long".to_string(),
            FfiType::RustBuffer(_) => "MemorySegment".to_string(),
            FfiType::RustCallStatus => "MemorySegment".to_string(),
            FfiType::ForeignBytes => "MemorySegment".to_string(),
            FfiType::Callback(_) => "MemorySegment".to_string(),
            FfiType::Struct(_) => "MemorySegment".to_string(),
            FfiType::Reference(_) | FfiType::MutReference(_) => "MemorySegment".to_string(),
            FfiType::VoidPointer => "MemorySegment".to_string(),
        }
    }

    /// Generate primitive types where possible. In FFM, primitives map to Java primitives
    /// and compound types map to MemorySegment.
...
mod filters {
    ...
    use uniffi_bindgen::interface::ffi::{ExternalFfiMetadata, FfiCallbackFunction};
    ...
    /// Maps an FfiType to its ValueLayout constant for use in FunctionDescriptor construction
    pub fn ffi_value_layout(type_: &FfiType) -> Result<String, askama::Error> {
        Ok(match type_ {
            FfiType::Int8 | FfiType::UInt8 => "ValueLayout.JAVA_BYTE".to_string(),
            FfiType::Int16 | FfiType::UInt16 => "ValueLayout.JAVA_SHORT".to_string(),
            FfiType::Int32 | FfiType::UInt32 => "ValueLayout.JAVA_INT".to_string(),
            FfiType::Int64 | FfiType::UInt64 => "ValueLayout.JAVA_LONG".to_string(),
            FfiType::Float32 => "ValueLayout.JAVA_FLOAT".to_string(),
            FfiType::Float64 => "ValueLayout.JAVA_DOUBLE".to_string(),
            FfiType::Handle => "ValueLayout.JAVA_LONG".to_string(),
            FfiType::RustArcPtr(_) => "ValueLayout.JAVA_LONG".to_string(),
            ...
            FfiType::Callback(_) => "ValueLayout.ADDRESS".to_string(),
            FfiType::Struct(name) => format!("{}.LAYOUT", JavaCodeOracle.ffi_struct_name(name)),
            FfiType::Reference(_) | FfiType::MutReference(_) => {
                "ValueLayout.ADDRESS".to_string()
            }
            FfiType::VoidPointer => "ValueLayout.ADDRESS".to_string(),
        })
    }

    /// Like ffi_value_layout but returns the UNALIGNED variant of ValueLayouts.
    /// Used for struct field access (get/set) where the segment base address
    /// may not be properly aligned (e.g., callback out-return parameters).
    /// StructLayouts (RustBuffer, RustCallStatus, etc.) are not returned here
    /// since they use slice-based (copy) access which doesn't check alignment.
    pub fn ffi_value_layout_unaligned(type_: &FfiType) -> Result<String, askama::Error> {
        Ok(match type_ {
            FfiType::Int8 | FfiType::UInt8 => "ValueLayout.JAVA_BYTE".to_string(),
            FfiType::Int16 | FfiType::UInt16 => "ValueLayout.JAVA_SHORT_UNALIGNED".to_string(),
            FfiType::Int32 | FfiType::UInt32 => "ValueLayout.JAVA_INT_UNALIGNED".to_string(),
            FfiType::Int64 | FfiType::UInt64 => "ValueLayout.JAVA_LONG_UNALIGNED".to_string(),
            FfiType::Float32 => "ValueLayout.JAVA_FLOAT_UNALIGNED".to_string(),
            FfiType::Float64 => "ValueLayout.JAVA_DOUBLE_UNALIGNED".to_string(),
            FfiType::Handle => "ValueLayout.JAVA_LONG_UNALIGNED".to_string(),
            FfiType::RustArcPtr(_) => "ValueLayout.JAVA_LONG_UNALIGNED".to_string(),
            FfiType::Callback(_) => "ValueLayout.ADDRESS_UNALIGNED".to_string(),
            FfiType::VoidPointer => "ValueLayout.ADDRESS_UNALIGNED".to_string(),
            FfiType::Reference(_) | FfiType::MutReference(_) => {
                "ValueLayout.ADDRESS_UNALIGNED".to_string()
            }
            // Embedded structs use slice-based access, not get/set
            other => ffi_value_layout(other)?,
        })
    }

    /// Provides the cast for invokeExact() return values
    pub fn ffi_invoke_exact_cast(type_: &FfiType) -> Result<String, askama::Error> {
        Ok(match type_ {
            FfiType::Int8 | FfiType::UInt8 => "(byte) ".to_string(),
            FfiType::Int16 | FfiType::UInt16 => "(short) ".to_string(),
            FfiType::Int32 | FfiType::UInt32 => "(int) ".to_string(),
    ...
    /// Returns true if the FfiType is a struct type (returns by value requiring SegmentAllocator)
    pub fn ffi_type_is_struct(type_: &FfiType) -> Result<bool, askama::Error> {
        Ok(matches!(
            type_,
            FfiType::RustBuffer(_)
                | FfiType::RustCallStatus
    ...
    /// Extract the struct class name from an FfiType::Struct
    pub fn ffi_struct_type_name(type_: &FfiType) -> Result<String, askama::Error> {
        match type_ {
            FfiType::Struct(name) => Ok(JavaCodeOracle.ffi_struct_name(name)),
            _ => Err(to_askama_error(&format!(
                "Expected FfiType::Struct, got {:?}",
                type_
            ))),
        }
    }

    /// Returns true if the FfiType is an embedded struct type (uses StructLayout, not ValueLayout).
    /// These types require slice-based access instead of VarHandle.
    pub fn ffi_type_is_embedded_struct(type_: &FfiType) -> Result<bool, askama::Error> {
        Ok(matches!(
            type_,
            FfiType::RustBuffer(_) | FfiType::RustCallStatus | FfiType::ForeignBytes
        ))
    }

    /// Alignment requirement for an FFI type in bytes (64-bit target).
    fn ffi_type_alignment(type_: &FfiType) -> usize {
        match type_ {
    ...
    /// Size of an FFI type in bytes (64-bit target).
    fn ffi_type_size(type_: &FfiType) -> usize {
        match type_ {
            FfiType::Int8 | FfiType::UInt8 => 1,
            FfiType::Int16 | FfiType::UInt16 => 2,
            FfiType::Int32 | FfiType::UInt32 | FfiType::Float32 => 4,
            FfiType::Int64 | FfiType::UInt64 | FfiType::Float64
            | FfiType::Handle | FfiType::RustArcPtr(_) => 8,
            FfiType::VoidPointer | FfiType::Callback(_)
            | FfiType::Reference(_) | FfiType::MutReference(_) => 8,
            FfiType::RustBuffer(_) => 24,
            FfiType::RustCallStatus => 32,
            FfiType::ForeignBytes => 16,
            FfiType::Struct(_) => {
                panic!("Cannot compute size of FfiType::Struct at codegen time")
            }
        }
    }

    /// Generates the struct layout body with proper padding between fields.
    pub fn ffi_struct_layout_fields(
        ffi_struct: &uniffi_bindgen::interface::FfiStruct,
    ) -> Result<String, askama::Error> {
        let mut parts = Vec::new();
        let mut current_offset: usize = 0;
        let fields = ffi_struct.fields();

        for field in fields {
            let ffi_type = field.type_();
            let alignment = ffi_type_alignment(&ffi_type);
            let padding = (alignment - (current_offset % alignment)) % alignment;

            if padding > 0 {
                parts.push(format!("MemoryLayout.paddingLayout({})", padding));
                current_offset += padding;
            }

            let layout = ffi_value_layout(&ffi_type)?;
            let name = JavaCodeOracle.var_name_raw(field.name());
            parts.push(format!("{}.withName(\"{}\")", layout, name));
            current_offset += ffi_type_size(&ffi_type);
        }

        // Tail padding to make struct size a multiple of its alignment
        if !fields.is_empty() {
            let max_alignment = fields
                .iter()
                .map(|f| ffi_type_alignment(&f.type_()))
                .max()
                .unwrap_or(1);
            let tail_padding = (max_alignment - (current_offset % max_alignment)) % max_alignment;
            if tail_padding > 0 {
                parts.push(format!("MemoryLayout.paddingLayout({})", tail_padding));
            }
        }
    ...
    /// For async callback interface methods: extracts the completion callback class name
    /// from the vtable method callback's parameters (the one with FfiType::Callback).
    pub fn async_completion_callback_name(
        ffi_callback: &FfiCallbackFunction,
    ) -> Result<String, askama::Error> {
        for arg in ffi_callback.arguments() {
            if let FfiType::Callback(name) = arg.type_() {
                return Ok(JavaCodeOracle.ffi_callback_name(&name));
            }
        }
        Err(to_askama_error(
            "No completion callback parameter found in async callback arguments",
        ))
    }
